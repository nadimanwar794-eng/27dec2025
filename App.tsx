...
const App: React.FC = () => {
  ...
  const [state, setState] = useState<AppState>({
    ...
    settings: {
        ...
        welcomeMessage: 'Experience the power of AI-driven education. IIC ONLINE AI filters out the noise of traditional textbooks to deliver only the essential, high-yield topics you need for success. Study smarter, not harder.',
        marqueeLines: ["Welcome to IIC ONLINE AI", "Learn Smart", "Contact Admin for Credits"], 
        liveMessage1: '', 
        liveMessage2: '', 
  ...
  useEffect(() => {
    ...
    // ALWAYS START INTERVAL IF USER IS LOGGED IN
    let interval: any;
    if (state.user) {
        interval = setInterval(() => {
            setDailyStudySeconds(prev => {
                ...
                if (state.user) {
                    ...
                    // Credit Rewards
                    if (next === 600) { creditsToAdd = 2; message = "10 mins studied! +2 Credits"; } // 10 min
                    else if (next === 1800) { creditsToAdd = 4; message = "30 mins studied! +4 Credits"; } // 30 min
                    else if (next === 3600) { creditsToAdd = 10; message = "1 Hour studied! +10 Credits"; } // 1 Hour
                    else if (next === 7200) { creditsToAdd = 20; message = "2 Hours studied! +20 Credits"; } // 2 Hours
                    else if (next === 10800) { creditsToAdd = 30; message = "3 Hours studied! +30 Credits"; } // 3 Hours
                    ...
                    if (creditsToAdd > 0) {
                        ...
                        let updatedUser = { ...state.user, credits: newCredits };
                        
                        // Add Inbox Message
                        const rewardMsg = {
                            id: `usage-reward-${Date.now()}`,
                            text: `🎉 ${message}`,
                            date: new Date().toISOString(),
                            read: false,
                            type: 'REWARD',
                            isClaimed: true // Auto-claimed for credits
                        };
                        updatedUser.inbox = [rewardMsg, ...(updatedUser.inbox || [])];

                        // SUBSCRIPTION REWARDS
                        if (next === 3600) { // 1 Hour -> Basic 4 Hours
                            const endDate = new Date(Date.now() + 4 * 60 * 60 * 1000).toISOString();
                            // Only apply if user doesn't have a better plan
                            if (!state.user.isPremium || state.user.subscriptionTier === 'FREE') {
                                updatedUser.subscriptionTier = 'WEEKLY'; // Mapped to Basic
                                updatedUser.subscriptionLevel = 'BASIC';
                                updatedUser.subscriptionEndDate = endDate;
                                updatedUser.isPremium = true;
                                updatedUser.grantedByAdmin = true;
                                
                                updatedUser.inbox.unshift({
                                    id: `sub-reward-1h-${Date.now()}`,
                                    text: "🎁 You unlocked 4 Hours of BASIC Subscription for studying 1 Hour!",
                                    date: new Date().toISOString(),
                                    read: false,
                                    type: 'REWARD',
                                    isClaimed: true
                        ...
                        if (next === 10800) { // 3 Hours -> Ultra 4 Hours
                            const endDate = new Date(Date.now() + 4 * 60 * 60 * 1000).toISOString();
                            updatedUser.subscriptionTier = 'MONTHLY'; // Mapped to Ultra
                            ...
                            updatedUser.inbox.unshift({
                                id: `sub-reward-3h-${Date.now()}`,
                                text: "🏆 AMAZING! You unlocked 4 Hours of ULTRA Premium for studying 3 hours!",
                                date: new Date().toISOString(),
                                read: false,
                                type: 'REWARD',
                                isClaimed: true
  ...
  }, [state.user?.id]); // Removed state.view dependency to run globally
  ...
  const handleLogin = (user: User) => {
    // LOGIN REWARD: 1 CREDIT (Once per day)
    const today = new Date().toDateString();
    const lastLogin = user.lastLoginDate ? new Date(user.lastLoginDate).toDateString() : '';
    
    let updatedUser = { ...user };
    
    if (lastLogin !== today && user.role !== 'ADMIN') {
        updatedUser.credits = (updatedUser.credits || 0) + 1;
        updatedUser.lastLoginDate = new Date().toISOString();
        alert("🎉 Login Bonus: +1 Credit!");
        
        // Add to Inbox
        const msg = {
            id: `login-bonus-${Date.now()}`,
            text: "✅ Daily Login Bonus: +1 Credit received.",
            date: new Date().toISOString(),
            read: false,
            type: 'REWARD',
            isClaimed: true
        };
        updatedUser.inbox = [msg, ...(updatedUser.inbox || [])];
    }

    localStorage.setItem('nst_current_user', JSON.stringify(updatedUser));
    saveUserToLive(updatedUser); // छात्र का डेटा क्लाउड पर भेजें
    localStorage.setItem('nst_has_seen_welcome', 'true');
    setState(prev => ({ ...prev, user: updatedUser, view: updatedUser.role === 'ADMIN' ? 'ADMIN_DASHBOARD' : 'STUDENT_DASHBOARD' as any, selectedBoard: updatedUser.board || null, selectedClass: updatedUser.classLevel || null, selectedStream: updatedUser.stream || null, language: updatedUser.board === 'BSEB' ? 'Hindi' : 'English', showWelcome: false }));
  };
      ...
      <header className="bg-white sticky top-0 z-30 shadow-sm border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
           <div onClick={() => setState(prev => ({ ...prev, view: state.user?.role === 'ADMIN' ? 'ADMIN_DASHBOARD' : 'STUDENT_DASHBOARD' as any }))} className="flex items-center gap-2 cursor-pointer">
               ...
               <h1 className="text-xl font-black text-slate-800">IIC ONLINE</h1>
           </div>
           {state.user && (
