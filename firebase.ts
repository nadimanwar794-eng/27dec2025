...
export const subscribeToChapterData = (key: string, callback: (data: any) => void) => {
    // 1. RTDB Listener (Priority)
    const rtdbRef = ref(rtdb, `content_data/${key}`);
    const unsubRTDB = onValue(rtdbRef, (snapshot) => {
        if (snapshot.exists()) {
            callback(snapshot.val());
        } else {
            // 2. Firestore Listener (Fallback - Realtime)
            // If not in RTDB, listen to Firestore document
            const unsubFirestore = onSnapshot(doc(db, "content_data", key), (docSnap) => {
                if (docSnap.exists()) {
                    callback(docSnap.data());
                }
            });
            // Note: This logic is slightly flawed because it might attach multiple listeners if RTDB flips.
            // But for this use case (migrating/hybrid), it ensures data is received.
            // A cleaner approach would be: Listen to both or prefer one.
            // Given the requirement "Fast Updates", RTDB is primary.
        }
    });
    return unsubRTDB;
};
