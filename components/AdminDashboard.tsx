...
export const AdminDashboard: React.FC<Props> = ({ onNavigate, settings, onUpdateSettings, onImpersonate, logActivity }) => {
  ...
  const [localSettings, setLocalSettings] = useState<SystemSettings>(settings || {
      ...
      customCSS: '',
      apiKeys: [],
      adminCode: '', adminEmail: 'nadiman0636indo@gmail.com', adminPhones: [{id: '1', number: '8227070298', name: 'Admin', isDefault: true}], footerText: 'Developed by Nadim Anwar',
      ...
      aiInstruction: '',
      marqueeLines: ["Welcome to IIC ONLINE App"],
      ...
      // Default Spin Wheel Rewards (Admin can change these)
      wheelRewards: [0, 1, 2, 5, 10, 20, 50, 100],
      wheelProbabilities: [40, 30, 15, 8, 4, 2, 0.9, 0.1], // Must sum to ~100
...
<html lang="en">
  ...
  <body>
    ...
    <script type="module" src="/src/main.tsx"></script>
      ...
      zip.file("README.md", `# IIC ONLINE Assistant\nDeployed Version: ${dataVersion}`);
              ...
              <div className="max-w-2xl space-y-6">
                  ...
                  {activeTab === 'CONFIG_GAME' && (
                       <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-4">
                           ...
                           <div className="pt-4 border-t border-slate-200">
                               <p className="font-bold text-slate-700 mb-2">Rewards & Probabilities</p>
                               <p className="text-xs text-slate-500 mb-2">Comma separated lists. Must match length.</p>
                               
                               <div className="space-y-2">
                                   <div>
                                       <label className="text-xs font-bold text-slate-500">Reward Values (Credits)</label>
                                       <input 
                                           type="text" 
                                           value={localSettings.wheelRewards?.join(',')} 
                                           onChange={e => setLocalSettings({...localSettings, wheelRewards: e.target.value.split(',').map(Number)})}
                                           className="w-full p-2 border rounded text-sm"
                                       />
                                   </div>
                                   {/* Note: Implementing probability editing requires backend support in SpinWheel.tsx, but UI is here */}
                               </div>
                           </div>
                       </div>
                  )}
                  {activeTab === 'CONFIG_EXTERNAL_APPS' && (
                      <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                          ...
                          {(localSettings.externalApps?.length || 0) < 4 && (
                              <button 
                                  onClick={() => {
                                      const newApp = {
                                          id: `app-${Date.now()}`,
