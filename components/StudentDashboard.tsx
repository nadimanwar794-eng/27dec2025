...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest }) => {
  ...
  return (
    <div>
        ...
        <div className={`grid grid-cols-4 gap-2 bg-white p-4 rounded-xl border border-slate-200 shadow-sm mb-8 sticky top-20 z-20`}>
            ...
            <button onClick={() => {
                const isUnlocked = localStorage.getItem(`reward_page_unlocked_${user.id}`);
                if (isUnlocked) {
                    setActiveTab('REDEEM');
                } else {
                    if (confirm("Unlock Reward Page for 10 Coins?")) {
                        if (user.credits >= 10) {
                            const updated = { ...user, credits: user.credits - 10 };
                            handleUserUpdate(updated);
                            localStorage.setItem(`reward_page_unlocked_${user.id}`, 'true');
                            setActiveTab('REDEEM');
                        } else {
            ...
            }} className={`flex flex-col items-center justify-center p-2 rounded-lg transition-all min-w-[60px] ${activeTab === 'REDEEM' ? 'bg-slate-100 text-[var(--primary)]' : 'text-slate-400 hover:bg-slate-50'}`}><Gift size={18} className="mb-1" /><span className="text-[9px] font-bold uppercase">Redeem</span></button>
            ...
            {activeTab === 'WEEKLY_TEST' && (
                <div className="animate-in fade-in zoom-in duration-300">
                    <div className="bg-gradient-to-r from-red-500 to-pink-500 rounded-2xl p-6 text-center text-white mb-6">
                        <h2 className="text-2xl font-bold mb-2">Exclusive MCQ</h2>
